...
| `sample.vhd` または `sample.vhdx` | プロジェクト直下に置くと、起動時にプライマリ ATA マスタとして自動接続されます（`vhd` を優先）。存在しなければディスク未接続として起動します。 |
...
| `snapshot.bin`（自動生成） | 実行状態のスナップショット。詳細は次項。 |

### 実行の再開（スナップショット）

1 命令ずつ解釈実行するため、ブートが完了するまで長時間かかることがあります。そのため実行中は `SnapshotInterval`（既定 1,000 万命令）ごとに CPU 状態と RAM/I/O ポート/CMOS/PIT/ATA レジスタ一式を `snapshot.bin` へ自動保存します（一時ファイルに書いてから置き換えるため、保存中にプロセスを強制終了してもファイルは壊れません）。
...
途中でプロセスを終了しても、次回 `--resume` を付けて起動すれば直近の自動保存地点から再開できます。`--resume` を付けない場合は常に `F000:FFF0` から新規に起動します（既存の `snapshot.bin` は無視され、実行に伴って上書きされていきます）。ディスクイメージ自体の内容（`sample.avhdx` への書き込み）はスナップショットと独立して随時ファイルへ反映されるため、スナップショットには含まれません。
...
実行は `InstructionLimit`（既定 5 億命令）に達するか、未実装オペコード・例外に遭遇するまで続きます。1 命令ずつ状態モナドを合成して解釈するため、ブートローダーの展開処理のような計算量の大きい区間では実 CPU に比べて非常に多くの命令数・実行時間を要します。実行中は一定間隔（`SnapshotInterval`）ごとに `CPU.WriteTo`/`EmuEnvironment.SaveState` で状態を `snapshot.bin` へ保存し、`--resume` で `CPU.ReadFrom`/`EmuEnvironment.LoadState` から続きを再開できます。
